...
class chainMap:
  ...
  def construct(self, PATH, ang_cutoff=4, noc=2):
    ...
    self.structSeq, self.resNums, self.transDict, self.adjacencyDict = self.return_struct_sequence_info(df, chain)
  ...
  def return_struct_sequence_info(self, df, chain):
    """
    ...
    """
    df_single_chain = df[df['Chain1']==chain]
    df_single_chain = df_single_chain[df_single_chain['Chain2']==chain]
    ...
    resNums = list(range(resNums_temp[0], resNums_temp[-1]+1)) #not sure why, but the list with range ends one number prematurely
    #print(resNums)
  ...
  def print_fasta_feature_files(self, featureStart, featureEnd, contactFromStart, contactFromEnd, fastaHeader, contactLabel, outputFileBase, threshold = 1, append = True, color = '117733', use_ref_seq_aligned=True):
    ...
    minRes = self.return_min_residue()-1  
...
def return_single_chain_dict(PDB_descriptive_df, PDB_ID, PATH, entity, chain):
  """
  ...
  cm_dict['cm'] = chainMap(PDB_ID, chain)
  cm_dict['cm'].construct(PATH, 5)
...
def return_interChain_adj(PATH, from_dict, to_dict, ang_cutoff=4, noc=2):
    """
    ...
    chains = [from_dict['chain'], to_dict['chain']]
    df_a_to_b = df[(df['Chain1']==from_dict['chain']) & (df['Chain2']==to_dict['chain'])]
    df_b_to_a = df[(df['Chain2']==from_dict['chain']) & (df['Chain1']==to_dict['chain'])]
    ...
    dict_combined = {}
    dict_combined[from_dict['chain']] = from_dict 
    dict_combined[to_dict['chain']] = to_dict
    adjList = {}
    for name, group in df_multichain.groupby(['ResNum1', 'ResNum2']):
        chain1 = group['Chain1'].iloc[0]
        chain2 = group['Chain2'].iloc[0]
...
def copy_chainMap(chain):
  ...
  """
  chain_copy = chainMap(chain.PDB_ID, chain.chain)
...
def translate_chainMap_to_RefSeq(chain, pdbClass):
  """
  ...
  chainMap_aligned = copy_chainMap(chain)

  match, offset = find_offset(chain, pdbClass)
  n_term_gaps = 0
  c_term_gaps = 0
  c_term_excess = 0
  n_term_excass = 0
