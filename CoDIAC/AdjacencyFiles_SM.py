...
def ProcessArpeggio(input_file_path, outfile_path, mmCIF_file, small_molecule = False):
       ...
       # filepath = self.PATH + '/' + self.input_json
        ...
        # if small_molecule:
        #     outfile = outfile_path+'/'+PDB_ID+'_SM.txt'
            ...
            with open(outfile,'w') as f:
                ...
                for i in range(len(data)):
                    ...
                    if small_molecule == False:
                        ...
                        if res_1 not in het_res:               
                            if res_2 not in het_res:                
...
def BinaryFeatures(PDB_ID, PATH, translate_resid=False, small_mol = False):
    ...
    # json_filepath = PATH+'/'+PDB_ID+'.json'
    # cif_file = PATH+'/'+PDB_ID+'.cif'
    # interpro_Adj = Interprotein_AdjFile(PDB_ID, PATH, translate_resid, small_molecule=small_mol)
    # intrapro_Adj = Intraprotein_AdjFile(PDB_ID, PATH, translate_resid, small_molecule=small_mol)
    ...
    # if small_mol ==True:
    #     print('Adjacency File with Binary features for polymer and non-polymer entities is generated for', PDB_ID)
    ...
    if small_mol:
        nonpolymer_adj = Nonpolymer_AdjFile(PDB_ID, PATH)
        df = pd.concat([intrapro_Adj, interpro_Adj, nonpolymer_adj])
    else:
        df = pd.concat([intrapro_Adj, interpro_Adj])
...
def Intraprotein_AdjFile(PDB_ID, PATH, translate_resid=False):
    ...
    df_check = pd.read_csv(INPUTFILE, sep='\t')
    smallmol = hetero_info(PDB_ID)
    smallmol_res_list = set(list(smallmol.keys()))
    adj_res_list = set(df_check['Res1'].tolist() + df_check['Res2'].tolist())
    adj_res_filtered = adj_res_list - smallmol_res_list

    df = df_check[(df_check['Res1'].isin(adj_res_filtered)) & (df_check['Res2'].isin(adj_res_filtered))]
...
# def Intraprotein_AdjFile(PDB_ID, PATH, translate_resid=False, small_molecule=False):
#     ''' Produces adjacency file for only Intraprotein contacts along with binary features
#     Parameters
#     ----------
#         PDB_ID : str
...
#     # if small_molecule == False:
#     #     smallmol = hetero_info(PDB_ID)
#     #     df_input = pd.read_csv(INPUTFILE, sep='\t')
#     #     df = pd.DataFrame()
        
#     #     for mol in smallmol:
#     #         # molname = smallmol[data][2]
#     #         filtered_df = df_input[(df_input["Res1"].str.contains(mol)==False) & (df_input["Res2"].str.contains(mol)==False)]
#     #         if len(filtered_df) != len(df_input):
...
#     for mol in smallmol:
#         if mol in set(df_check['Res1'].tolist()) or mol in set(df_check['Res2'].tolist()):
#             smallmol_bool = True
...
#     if small_molecule == smallmol_bool:
#         df = pd.read_csv(INPUTFILE, sep='\t')
        
#     if small_molecule != smallmol_bool:
        
#         df_input = pd.read_csv(INPUTFILE, sep='\t')
#         df = df_input[(df_input['Res1'].isin(adj_res_filtered)) & (df_input['Res2'].isin(adj_res_filtered))]
...
# def Interprotein_AdjFile(PDB_ID,PATH, translate_resid=False, small_molecule = False):
#     ''' Produces adjacency file for only Interprotein contacts along with binary features
#     Parameters
#     ----------
#         PDB_ID : str
...
def Nonpolymer_AdjFile(PDB_ID,PATH, translate_resid=False):
    INPUTFILE = PATH+'/'+PDB_ID+'.txt'
    #OUTFILE = PATH+'/'+PDB_ID+'/'+PDB_ID+'_inter.txt'

    df = pd.read_csv(INPUTFILE, sep='\t')
    # df = pd.read_csv('Data/1B55.txt', sep='\t')
    smallmol = hetero_info(PDB_ID)
    if smallmol:
        pol_entity_dict = entity_for_chain(PDB_ID)[0]
        resnum_dict = hetero_dict(PDB_ID, PATH)
        dbref_dict_upd = ResID_from_DBREF(PDB_ID)[1]
        dbref_dict = ResID_from_DBREF(PDB_ID)[2]
        
        nonpol_df = pd.DataFrame()
        
        for mol in smallmol:
            # molname = smallmol[data][2]
            filtered_df = df[(df["Res1"].str.contains(mol)) | (df["Res2"].str.contains(mol))]
            nonpol_df = pd.concat([nonpol_df, filtered_df], ignore_index = True) 
            
        nonpol_df = nonpol_df.drop_duplicates()
        
        entity_list_res1 = []
        resnum_upd_1 = []
        for i in range(len(nonpol_df)):
            resname = nonpol_df['Res1'][i]
            if resname in smallmol.keys():
                entity = smallmol[resname][2]
                entity_list_res1.append(entity)
        
                resnum_pair = nonpol_df['Chain1'][i]+str(nonpol_df['ResNum1'][i])
                resnum_upd = resnum_dict[resnum_pair]
                resnum_upd_1.append(resnum_upd)
            else:
                chain = nonpol_df['Chain1'][i]
                pol_entity = pol_entity_dict[chain]
                entity_list_res1.append(pol_entity)
                
                resnum_pair = nonpol_df['Chain1'][i]+str(nonpol_df['ResNum1'][i])
                if translate_resid == True:
                    resnum_upd = dbref_dict_upd[resnum_pair]
                    resnum_upd_1.append(resnum_upd)
                if translate_resid == False:
                    resnum_upd = dbref_dict[resnum_pair]
                    resnum_upd_1.append(resnum_upd)
        ...
        for i in range(len(nonpol_df)):
            ...
            resname = nonpol_df['Res2'][i]
            if resname in smallmol.keys():
                entity = smallmol[resname][2]
                entity_list_res2.append(entity)
        
                resnum_pair = nonpol_df['Chain2'][i]+str(nonpol_df['ResNum2'][i])
                resnum_upd = resnum_dict[resnum_pair]
                resnum_upd_2.append(resnum_upd)
            else:
                chain = nonpol_df['Chain2'][i]
                pol_entity = pol_entity_dict[chain]
                entity_list_res2.append(pol_entity)
        ...
        nonpol_df['ResNum1_upd'] = resnum_upd_1
        nonpol_df['ResNum2_upd'] = resnum_upd_2
        nonpol_df['Entity1'] = entity_list_res1
        nonpol_df['Entity2'] = entity_list_res2
    
        nonpol_df['Entity Pair'] = nonpol_df['Entity1'].astype(str)+'-'+(nonpol_df['Entity2']).astype(str)
        nonpol_df['ResNum Pair'] = nonpol_df['ResNum1_upd'].astype(str)+'-'+(nonpol_df['ResNum2_upd']).astype(str)
        nonpol_df['Chain Pair'] = nonpol_df['Chain1'].astype(str)+'-'+(nonpol_df['Chain2']).astype(str)
        
        threshold = interprotein_threshold(PDB_ID)
        
        df2 = nonpol_df[nonpol_df['Entity1']!=nonpol_df['Entity2']]
        df2 = df2.reset_index(drop=True)
        df2.insert(11,'Binary_Feature','')
...
def hetero_info(PDB_ID):
    hetero_info_dict = {}
   
    response1 = requests.get(f'https://data.rcsb.org/rest/v1/core/entry/{PDB_ID}').json()
    if 'non_polymer_entity_ids' in response1['rcsb_entry_container_identifiers'].keys():
        entity_nonpol = response1['rcsb_entry_container_identifiers']['non_polymer_entity_ids']
    else:
        entity_nonpol= []
    
    for entity_id in entity_nonpol:
        nonpolymer_entity_url = "https://data.rcsb.org/rest/v1/core/nonpolymer_entity/" + \
                            PDB_ID + "/" + entity_id
        
        response2 = requests.get(nonpolymer_entity_url).json()
        chains = response2['rcsb_nonpolymer_entity_container_identifiers']['auth_asym_ids']
        description = response2['rcsb_nonpolymer_entity']['pdbx_description']
        name = response2['rcsb_nonpolymer_entity_container_identifiers']['chem_ref_def_id']
        hetero_info_dict[name] = [chains, description, entity_id]
        
    return(hetero_info_dict)

def hetero_dict(PDB_ID, PATH):
    mmCIF_file = PATH+'/'+PDB_ID+'.cif'
    pdb_info = MMCIF2Dict(mmCIF_file)
    het_dict = {}
   
    if '_pdbx_nonpoly_scheme.pdb_mon_id' in pdb_info.keys():
        res = pdb_info['_pdbx_nonpoly_scheme.pdb_mon_id']
        for i in range(len(res)):
            resid = pdb_info['_pdbx_nonpoly_scheme.pdb_seq_num'][i]
            chain = pdb_info['_pdbx_nonpoly_scheme.pdb_strand_id'][i]
            key = (chain)+str(resid)
            het_dict[key] = int(resid)
    return het_dict
